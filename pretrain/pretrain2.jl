...
let
    ...
    ys = Array{Float32}(undef, Nelec, Nelec, samples)
    for i in 1:samples
        orbitals = collect(Int32, 1:L)
        selected_orbital = shuffle(orbitals)[1:Nelec]
        xs[:, i] = Int32.(selected_orbital)
        ys[:, :, i] = real.(10 * ψ.Mocoeff[selected_orbital, :])
    end

    # @show mean(ys)
    # @show ys

    ψ = f32(SlaterNet(; emb_size=10, Nx=Nx, Ny=Ny, Nelec=Nelec))


    model_filename = "./data/test10.bson"
    ...
    function loss_fn(m, X, Y)
        ...
        # Losses = [Flux.Losses.mse(y[:, :, i], Y[:, :, i]) for i in 1:samples]
        #

        return mean(Flux.Losses.mse(y, Y; agg=sum))
    end
    ...
    for i in 1:5
        ...
        y_exact = logabsdet(ys[:, :, i])
        y_model = logabsdet(ψ(selected_orbitals))
        if i == 1
            @show ys[:, :, i]
            @show ψ(selected_orbitals)
        end
