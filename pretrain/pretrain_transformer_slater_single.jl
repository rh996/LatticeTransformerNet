...
let
    ...
    y = Array{Float64}(undef, length(configurations))
    for i in eachindex(configurations)
        y[i] = det(ψ.Mocoeff[configurations[i].Electrons, :])
    end


    y = Array{Float64}(undef, length(configurations))
    for i in eachindex(configurations)
        y[i] = det(10 * ψ.Mocoeff[configurations[i].Electrons, :])
    end
    @show mean(y)
    ...
    loss_fn(model, X, Y) = begin
        ...
        detlist = [det(y_pred_reshaped[:, :, i]) for i in 1:size(X, 2)]
        Flux.Losses.mse(detlist, Y)
    end
